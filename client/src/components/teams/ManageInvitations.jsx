...
class ManageInvitations extends Component {
  ...
  constructor(props) {
    super(props);
    this.handleWithdrawInvitationClick = this.handleWithdrawInvitationClick.bind(
      this
    );
  ...
  handleWithdrawConfirmClick() {
    const {
      withdrawInvitation,
      teamEvents: {
        invitations: { items, confirmWithdraw }
      }
    } = this.props;
  ...
  getFilteredInvitations() {
    ...
    return Object.keys(items)
      .map(invitationId => items[invitationId])
    ...
    return (
      invitation && (
        <Modal show={Boolean(confirmWithdraw)}>
          <Modal.Header>
            <Modal.Title>Confirm Withdraw Invitation</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
              Are you sure you want to withdraw{" "}
              <strong>
                <PlayerName playerId={invitation.player} />
                's
              </strong>{" "}
              invitation? They will no longer be able to join the team with this
              invitation. This cannot be undone.
            </p>
          </Modal.Body>
          <Modal.Footer>
              ...
              <tr key={invitation.id}>
                <td>
                  <Link to={`/players/${invitation.player}/`}>
                    <PlayerName
                      key={invitation.player}
                      playerId={invitation.player}
                    />
                  </Link>
                </td>
                <td>{positions.items[invitation.position].name}</td>
                  ...
                  {invitation.status === 1 && (
                    <ButtonToolbar>
                      <Button
                        ...
                        onClick={() =>
                          this.handleWithdrawInvitationClick(invitation.id)
                        }
                      >
                        Withdraw
                      </Button>
