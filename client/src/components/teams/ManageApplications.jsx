...
class ManageApplications extends Component {
  ...
  constructor(props) {
    super(props);
    this.handleAcceptApplicationClick = this.handleAcceptApplicationClick.bind(
      this
    );
    ...
    this.handleRejectApplicationClick = this.handleRejectApplicationClick.bind(
      this
    );
  ...
  handleAcceptConfirmClick() {
    const {
      acceptApplication,
      teamEvents: {
        applications: { items, confirmAccept }
      }
    } = this.props;
  ...
  handleRejectConfirmClick() {
    const {
      rejectApplication,
      teamEvents: {
        applications: { items, confirmReject }
      }
    } = this.props;
  ...
  getFilteredApplications() {
    ...
    return Object.keys(items)
      .map(applicationId => items[applicationId])
    ...
    return (
      application && (
        <Modal show={Boolean(confirmAccept)}>
          <Modal.Header>
            <Modal.Title>Confirm Accept Application</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
    ...
    return (
      application && (
        <Modal show={Boolean(confirmReject)}>
          <Modal.Header>
            <Modal.Title>Confirm Reject Application</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
  ...
  renderApplicationTab(index, statusLabel, statusIndex) {
    ...
    applications = applications.filter(
      application => application.status === statusIndex
    );
    return (
      <Tab
        eventKey={index}
        key={`${statusLabel}-applications-${index}`}
        title={
          <ApplicationTabLabel count={applications.length}>
            {statusLabel}
          </ApplicationTabLabel>
        }
      >
              ...
              <tr key={application.id}>
                <td>
                  <Link to={`/players/${application.player}/`}>
                    <PlayerName
                      key={application.player}
                      playerId={application.player}
                    />
                  </Link>
                </td>
                <td>{positions.items[application.position].name}</td>
                <td>{moment(application.created).format("L")}</td>
                <td>
                  {application.status === 1 && (
                    <ButtonToolbar>
                      <Button
                        ...
                        onClick={() =>
                          this.handleAcceptApplicationClick(application.id)
                        }
                      >
                        Accept
                      </Button>
                      <Button
                        ...
                        disabled={!playerIsCaptain(player, team)}
                        onClick={() =>
                          this.handleRejectApplicationClick(application.id)
                        }
                      >
                        Reject
                      </Button>
  ...
  render() {
    const {
      teamEvents: {
        applications: { isLoading, lastUpdated }
      }
    } = this.props;
