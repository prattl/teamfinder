...
class DeleteTeamMemberConfirmModal extends Component {
  ...
  render() {
    const {
      team: { confirmDeleteTeamMember, deleteTeamMemberError, team },
      player
    } = this.props;
    const teamMember = team.team_members.find(
      member => member.id === confirmDeleteTeamMember
    );
    const playerIsLeavingTeam =
      ...
      <Modal show={Boolean(confirmDeleteTeamMember)}>
        ...
        <Modal.Body>
          {deleteTeamMemberError && (
            <Alert bsStyle="danger">{deleteTeamMemberError}</Alert>
          )}
          {playerIsLeavingTeam ? (
            <p>
              Are you sure you want to leave <strong>{team.name}</strong>? This
              cannot be undone.
            </p>
          ) : (
            <p>
              Are you sure you want to remove{" "}
              <strong>{teamMember.player.username}</strong> from{" "}
              <strong>{team.name}</strong>? This cannot be undone.
            </p>
          )}
        </Modal.Body>
        <Modal.Footer>
          <Button
            bsStyle="link"
            onClick={() =>
              this.handleDeleteTeamMemberCancelClick(teamMember.id)
            }
          >
            Cancel
          </Button>
          <Button
            bsStyle="danger"
            onClick={() =>
              this.handleDeleteTeamMemberConfirmClick(
                teamMember.id,
                playerIsLeavingTeam
              )
            }
