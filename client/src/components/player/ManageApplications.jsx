...
class ManageApplications extends Component {
  ...
  constructor(props) {
    super(props);
    this.handleWithdrawApplicationClick = this.handleWithdrawApplicationClick.bind(
      this
    );
    this.handleWithdrawCancelClick = this.handleWithdrawCancelClick.bind(this);
    this.handleWithdrawConfirmClick = this.handleWithdrawConfirmClick.bind(
      this
    );
  }
  ...
  handleWithdrawConfirmClick() {
    const {
      withdrawApplication,
      playerEvents: {
        applications: { items, confirmWithdraw }
      }
    } = this.props;
    ...
    return (
      application && (
        <Modal show={Boolean(confirmWithdraw)}>
          <Modal.Header>
            <Modal.Title>Confirm Withdraw Application</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
              Are you sure you want to withdraw your application to{" "}
              <strong>
                <TeamName teamId={application.team} />
              </strong>
              ? This cannot be undone.
            </p>
          </Modal.Body>
          <Modal.Footer>
            <Button bsStyle="link" onClick={this.handleWithdrawCancelClick}>
              Cancel
            </Button>
              ...
              <tr key={application.id}>
                <td>
                  <Link to={`/teams/${application.team}/`}>
                    <TeamName
                      key={application.team}
                      teamId={application.team}
                    />
                  </Link>
                </td>
                <td>
                  {positions.items[application.position] &&
                  ...
                  {application.status === 1 && (
                    <ButtonToolbar>
                      <Button
                        bsStyle="danger"
                        onClick={() =>
                          this.handleWithdrawApplicationClick(application.id)
                        }
                      >
                        Withdraw
                      </Button>
