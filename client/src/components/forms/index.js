...
import {
  withAllFixtures,
  withPositions
} from "components/connectors/WithFixtures";
...
export const createGenericInput = (Component, label) => field => (
  ...
  >
    ...
    {field.meta.touched &&
      field.meta.error && <HelpBlock>{field.meta.error}</HelpBlock>}
  </FormGroup>
);
  ...
  >
    ...
    {maxLength && (
      <div className="text-right">
        {field.input.value.length}/{maxLength}
      </div>
    )}
    {field.meta.touched &&
      field.meta.error && <HelpBlock>{field.meta.error}</HelpBlock>}
  ...
  >
    ...
    >
      ...
      {Object.keys(positions.items).map(positionId => (
        <option key={`position-${positionId}-${meta.form}`} value={positionId}>
...
export const createS3UploadInput = props => {
  ...
  class _S3Upload extends Component {
    ...
    getSignedUrl(file, callback) {
      ...
      this.props
        .dispatch(submitLogoUpload(params))
        .then(json => {
          callback(json);
        })
        .catch(error => {
    ...
    handleUploadFinish(obj, file) {
      ...
      this.props.dispatch(
        change(
          "team",
          "logo_url",
          `https://dotateamfinder.s3.amazonaws.com/team-logos/${filename}`
        )
      );
    }
...
let RegionSelect = props => {
  const {
    ...
    multi = true,
    fixtures: {
      regions: { items, isLoading, lastUpdated }
    }
  } = props;
...
let PositionSelect = props => {
  const {
    ...
    multi = true,
    fixtures: {
      positions: { items, isLoading, lastUpdated }
    }
  } = props;
  const primaryOptions = Object.keys(items)
    .filter(itemId => !items[itemId].secondary)
    .map(itemId => ({
      value: itemId,
      label: items[itemId].name
    }));
  const secondaryOptions = Object.keys(items)
    .filter(itemId => items[itemId].secondary)
    .map(itemId => ({
      value: itemId,
      label: items[itemId].name
...
let InterestSelect = props => {
  const {
    ...
    multi = true,
    fixtures: {
      interests: { items, isLoading, lastUpdated }
    }
  } = props;
  const options = Object.keys(items).map(itemId => ({
    value: itemId,
...
let LanguageSelect = props => {
  const {
    ...
    multi = true,
    fixtures: {
      languages: { items, isLoading, lastUpdated }
    }
  } = props;
  const options = Object.keys(items).map(itemId => ({
    value: itemId,
