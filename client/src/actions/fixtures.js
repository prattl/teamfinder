  ...
  return fetch(createUrl(endpoint), {
    ...
    response.json().then(json => {
      const payload = response.ok
        ? json
        : new Error("Error retrieving fixture.");
      return dispatch(
        createAction(actions.RECEIVE_FIXTURE, null, metaGenerator)({
          fixtureType,
          json: payload
        })
      );
...
export const requestAllFixtures = () => (dispatch, getState) =>
  Object.keys(getState().fixtures).map(fixtureType =>
    dispatch(requestFixture(fixtureType))
  );
