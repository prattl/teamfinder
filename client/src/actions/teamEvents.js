...
export const requestTeamInvitations = id => (dispatch, getState) => {
  ...
  return GET(url, getState().auth.authToken).then(response =>
    response.json().then(json => {
      ...
      return dispatch(
        createAction(actions.RECEIVE_TEAM_INVITATIONS, null, metaGenerator)(
          payload
        )
      );
    })
...
export const requestTeamApplications = id => (dispatch, getState) => {
  ...
  return GET(url, getState().auth.authToken).then(response =>
    response.json().then(json => {
      const payload = response.ok
        ? json
        : new Error("Error retrieving team applications.");
      return dispatch(
        createAction(actions.RECEIVE_TEAM_APPLICATIONS, null, metaGenerator)(
          payload
        )
      );
    })
...
export const tryAcceptApplication = createAction(
  actions.CONFIRM_ACCEPT_APPLICATION
);
export const cancelAcceptApplication = createAction(
  actions.CANCEL_ACCEPT_APPLICATION
);
export const tryRejectApplication = createAction(
  actions.CONFIRM_REJECT_APPLICATION
);
export const cancelRejectApplication = createAction(
  actions.CANCEL_REJECT_APPLICATION
);
export const tryWithdrawInvitation = createAction(
  actions.CONFIRM_WITHDRAW_INVITATION
);
export const cancelWithdrawInvitation = createAction(
  actions.CANCEL_WITHDRAW_INVITATION
);

const updateTeamEventStatus = eventType => status => (eventId, teamId) => (
  dispatch,
  getState
) => {
  const actionTypes =
    eventType === TEAM_EVENT_TYPES.INVITATION
      ? [
          actions.REQUEST_UPDATE_TEAM_INVITATION_STATUS,
          actions.RECEIVE_UPDATE_TEAM_INVITATION_STATUS
        ]
      : [
          actions.REQUEST_UPDATE_TEAM_APPLICATION_STATUS,
          actions.RECEIVE_UPDATE_TEAM_APPLICATION_STATUS
        ];
  ...
  if (authToken) {
    return PATCH(
      createUrl(`/api/${eventType}/${eventId}/?team=${teamId}`),
      authToken,
      { status }
    ).then(response =>
      response.json().then(json => {
...
const updateApplicationStatus = updateTeamEventStatus(
  TEAM_EVENT_TYPES.APPLICATION
);
const updateInvitationStatus = updateTeamEventStatus(
  TEAM_EVENT_TYPES.INVITATION
);
