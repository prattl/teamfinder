      ...
      response =>
        response.json().then(json => {
          const payload = response.ok
            ? json
            : new Error("Error retrieving new items.");
          return dispatch(
            createAction(actions.RECEIVE_NEW_PLAYER_ITEMS)(payload)
          );
        })
    );
  }
    ...
    return GET(createUrl("/api/user_email_preferences/self/"), authToken).then(
      response =>
        response.json().then(json => {
          const payload = response.ok
            ? json
            : new Error("Error retrieving email preferences.");
          return dispatch(
            createAction(actions.RECEIVE_EMAIL_PREFERENCES)(payload)
          );
        })
    );
  }
...
export const submitEmailPreferences = data => (dispatch, getState) => {
  ...
  const {
    auth: { authToken },
    player: {
      userEmailPreferences: { id }
    }
  } = getState();
  if (id && authToken) {
    return PATCH(
      createUrl(`/api/user_email_preferences/${id}/`),
      authToken,
      data
    ).then(response =>
      response.json().then(json => {
        const payload = response.ok
          ? json
          : new Error("Error submitting email preferences.");
        dispatch(
          createAction(
            actions.RECEIVE_SUBMIT_EMAIL_PREFERENCES,
            null,
            metaGenerator
          )(payload)
        );
  ...
  return GET(createUrl("/api/players/me/"), getState().auth.authToken).then(
    response =>
      response.json().then(json => {
        const payload = response.ok
          ? json
          : new Error("Error retrieving own player.");
        dispatch(
          createAction(actions.RECEIVE_OWN_PLAYER, null, metaGenerator)(payload)
        );
        if (response.ok) {
    ...
    return PATCH(createUrl(`/api/players/${id}/`), authToken, data).then(
      response =>
        response.json().then(json => {
          const payload = response.ok
            ? json
            : new Error("Error submitting profile.");
          if (response.ok) {
            dispatch(requestOwnPlayer());
          }
          dispatch(
            createAction(actions.RECEIVE_SUBMIT_PROFILE, null, metaGenerator)(
              payload
            )
          );
          notify(response);
...
export const submitApplication = data => (dispatch, getState) => {
  ...
  if (id && authToken) {
    return POST(createUrl("/api/applications/"), authToken, {
      ...data,
      player: id
    }).then(response =>
      response.json().then(json => {
        const payload = response.ok
          ? json
          : new Error("Error creating application.");
        dispatch(
          createAction(actions.RECEIVE_SUBMIT_APPLICATION, null, metaGenerator)(
            payload
          )
        );
