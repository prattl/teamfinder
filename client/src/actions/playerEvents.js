...
export const requestPlayerInvitations = () => (dispatch, getState) => {
    ...
    return GET(url, getState().auth.authToken).then(response => response.json().then(json => {
        const payload = response.ok ? json : new Error('Error retrieving player invitations.')
        return dispatch(createAction(actions.RECEIVE_PLAYER_INVITATIONS, null, metaGenerator)(payload))
    }))
}
...
export const requestPlayerApplications = () => (dispatch, getState) => {
    ...
    return GET(url, getState().auth.authToken).then(response => response.json().then(json => {
        const payload = response.ok ? json : new Error('Error retrieving player applications.')
        return dispatch(createAction(actions.RECEIVE_PLAYER_APPLICATIONS, null, metaGenerator)(payload))
    }))
}

export const tryAcceptInvitation = createAction(actions.CONFIRM_ACCEPT_INVITATION)
export const cancelAcceptInvitation = createAction(actions.CANCEL_ACCEPT_INVITATION)
export const tryRejectInvitation = createAction(actions.CONFIRM_REJECT_INVITATION)
export const cancelRejectInvitation = createAction(actions.CANCEL_REJECT_INVITATION)
export const tryWithdrawApplication = createAction(actions.CONFIRM_WITHDRAW_APPLICATION)
export const cancelWithdrawApplication = createAction(actions.CANCEL_WITHDRAW_APPLICATION)

const updatePlayerEventStatus = eventType => status => (eventId) => (dispatch, getState) => {
    const actionTypes = eventType === PLAYER_EVENT_TYPES.INVITATION ? [
        actions.REQUEST_UPDATE_PLAYER_INVITATION_STATUS, actions.RECEIVE_UPDATE_PLAYER_INVITATION_STATUS
    ] : [
        actions.REQUEST_UPDATE_PLAYER_APPLICATION_STATUS, actions.RECEIVE_UPDATE_PLAYER_APPLICATION_STATUS
    ]
    ...
    if (authToken) {
        return PATCH(createUrl(`/api/${eventType}/${eventId}/`),
...
const updateApplicationStatus = updatePlayerEventStatus(PLAYER_EVENT_TYPES.APPLICATION)
const updateInvitationStatus = updatePlayerEventStatus(PLAYER_EVENT_TYPES.INVITATION)
