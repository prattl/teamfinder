...
export const submitCreateTeam = data => (dispatch, getState) => {
  ...
  if (authToken) {
    return POST(createUrl("/api/teams/"), authToken, data).then(response =>
      response.json().then(json => {
        const payload = response.ok ? json : new Error("Error creating team.");
        dispatch(
          createAction(actions.RECEIVE_SUBMIT_CREATE_TEAM, null, metaGenerator)(
            payload
          )
        );
        if (response.ok) {
      ...
      response =>
        response.json().then(json => {
          const payload = response.ok
            ? json
            : new Error("Error creating team.");
          dispatch(
            createAction(actions.RECEIVE_SUBMIT_EDIT_TEAM, null, metaGenerator)(
              payload
            )
          );
          notify(response);
...
export const tryDeleteTeamMember = createAction(
  actions.CONFIRM_DELETE_TEAM_MEMBER
);
export const cancelDeleteTeamMember = createAction(
  actions.CANCEL_DELETE_TEAM_MEMBER
);

export const tryPromoteToCaptain = createAction(
  actions.CONFIRM_PROMOTE_TO_CAPTAIN
);
export const cancelPromoteToCaptain = createAction(
  actions.CANCEL_PROMOTE_TO_CAPTAIN
);
  ...
  if (authToken) {
    return DELETE(createUrl(`/api/teams/${teamId}/`), authToken).then(
      response => {
        const payload = response.ok
          ? teamId
          : new Error("Error deleting team.");
        if (response.ok) {
          browserHistory.push("/teams/manage/");
        }
        dispatch(requestOwnPlayer());
        return dispatch(
          createAction(actions.RECEIVE_DELETE_TEAM, null, metaGenerator)(
            payload
          )
        );
      }
...
export const deleteTeamMember = (teamMemberId, teamId, leavingTeam = false) => (
  dispatch,
  getState
) => {
  ...
  if (authToken) {
    return DELETE(
      createUrl(`/api/memberships/${teamMemberId}/`),
      authToken
    ).then(response => {
      return response[response.status === 204 ? "text" : "json"]().then(
        json => {
          ...
          return dispatch(
            createAction(actions.RECEIVE_DELETE_TEAM_MEMBER, null, p => ({
              ...metaGenerator(p),
              teamMemberId,
              teamId // TODO: Putting this information in meta isn't ideal
            }))(payload)
          );
        }
      );
...
export const promoteToCaptain = (teamMemberId, teamId) => (
  dispatch,
  getState
) => {
  dispatch(
    createAction(actions.REQUEST_PROMOTE_TO_CAPTAIN)({ teamMemberId, teamId })
  );
  const {
    auth: { authToken }
  } = getState();
  if (authToken) {
    const { player } = getState().teams.teams[teamId].team.team_members.find(
      teamMember => teamMember.id === teamMemberId
    );
    return PATCH(createUrl(`/api/teams/${teamId}/`), authToken, {
      captain: player.id
    }).then(response =>
      response.json().then(json => {
        const payload = response.ok ? json : new Error(json.detail);
        return dispatch(
          createAction(actions.RECEIVE_PROMOTE_TO_CAPTAIN, null, p => ({
            ...metaGenerator(p),
            teamMemberId,
            teamId
          }))(payload)
...
export const submitEditTeamMember = (teamMemberId, data) => (
  dispatch,
  getState
) => {
  ...
  if (authToken) {
    return PATCH(
      ...
      response.json().then(json => {
        ...
        dispatch(
          createAction(actions.RECEIVE_EDIT_TEAM_MEMBER, null, p => ({
            ...metaGenerator(p),
            teamMemberId
          }))(payload)
        );
...
export const submitLogoUpload = params => (dispatch, getState) => {
  ...
  return GET(
    createUrl(`/s3/sign/?${querystring.stringify(params)}`),
    authToken
