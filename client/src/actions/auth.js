  ...
  return GET(createUrl("/api/auth/me/"), getState().auth.authToken).then(
    response =>
      response.json().then(json => {
        ...
        return dispatch(
          createAction(actions.RECEIVE_AUTH_STATUS, null, metaGenerator)(
            payload
          )
        );
      })
...
export const setAuthTokenFromSteamSignIn = token => (dispatch, getState) => {
  ...
  dispatch(
    createAction(actions.RECEIVE_LOGIN, null, metaGenerator)({
      auth_token: token
    })
  );
...
export const login = credentials => (dispatch, getState) => {
  ...
  return POST(
    ...
    response.json().then(json => {
      ...
      dispatch(
        createAction(actions.RECEIVE_LOGIN, null, metaGenerator)(payload)
      );
...
export const logout = () => (dispatch, getState) => {
  ...
  return POST(createUrl("/api/auth/logout/"), getState().auth.authToken).then(
    response => {
      const payload = response.ok
        ? null
        : new Error("Error submitting logout.");
      if (response.ok) {
        localStorage.setItem("authtoken", null);
      }
      return dispatch(
        createAction(actions.RECEIVE_LOGOUT, null, metaGenerator)(payload)
      );
    }
  );
};
  ...
  return POST(createUrl("/api/auth/register/"), null, credentials).then(
    response =>
      response.json().then(json => {
        const payload = response.ok
          ? json
          : new Error("Error submitting register.");
        dispatch(
          createAction(actions.RECEIVE_SIGNUP, null, metaGenerator)(payload)
        );
        if (response.ok) {
