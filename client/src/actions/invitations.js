...
export const requestInvitationsForSelf = () => (dispatch, getState) => {
    ...
    return GET(createUrl(`/api/invitations/?player=${id}`), authToken).then(
        response => response.json().then(json => {
            const payload = response.ok ? json : new Error('Error retrieving own invitations.')
            return dispatch(createAction(actions.RECEIVE_INVITATIONS_FOR_SELF, null, metaGenerator)(payload))
        })
    )
}
...
export const requestInvitationsForTeam = teamId => (dispatch, getState) => {
    ...
    return GET(createUrl(`/api/invitations/?team=${teamId}`), authToken).then(
        response => response.json().then(json => {
            ...
            return dispatch(createAction(actions.RECEIVE_INVITATIONS_FOR_TEAM, null, metaGenerator)(payload))
        })
    )
}
