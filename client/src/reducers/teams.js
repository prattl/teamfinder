...
const teams = handleActions(
  {
    ...
    [actions.REQUEST_TEAM]: (state, action) => ({
      ...
      teams: {
        ...state.teams,
        [action.payload]: Object.keys(state.teams).includes(action.payload)
    ...
    [actions.RECEIVE_DELETE_TEAM_MEMBER]: (state, action) => {
      const updatedTeamMembers = state.teams[
        action.meta.teamId
      ].team.team_members.filter(
        teamMember => teamMember.id !== action.meta.teamMemberId
      );
      return {
        ...state,
        teams: {
        ...
        teams: {
          ...
          [action.meta.teamId]: {
            ...
            team: action.error
              ? state.teams[action.meta.teamId].team
              : action.payload,
            confirmPromoteToCaptain: action.error
              ? state.teams[action.meta.teamId].confirmPromoteToCaptain
              : null,
            confirmPromoteToCaptainError: action.error
